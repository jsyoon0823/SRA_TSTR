...
#### For each CV Folds
    ...
    ### For each imputed data matrix
        ...
        for j in range(L):
            AUC_ar[j][k] = metrics.roc_auc_score(Y_test_New, Predict_av[:,j]/M)
            AUPRC_ar[j][k] = metrics.average_precision_score(Y_test_New, Predict_av[:,j]/M)
...
#%% AUROC / AUPRC Output
    ...
    # AUROC/AUPRC
    # Setting B
    file_name = '/home/jinsung/Documents/Jinsung/2018_Research/KDD/Result/' + data_name + '_p_' + str(int(100*prob)) + '_New.csv'
